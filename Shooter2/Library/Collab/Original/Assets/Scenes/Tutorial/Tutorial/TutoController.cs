...
//COMENTARIO DE PRUEBA A VER QUE CHCH PASA D:
...
public class TutoController : MonoBehaviour
{
    ...
    void posicionarLasers()
    {
        ...
        {
            RaycastHit hit;
            if (Physics.Raycast(lasersGame[i].transform.parent.position, new Vector3(-lasersGame[i].transform.parent.forward.x, 0f, -lasersGame[i].transform.parent.forward.z), out hit))
            {
                if (hit.collider && hit.transform.gameObject.tag == "Wall")
                {
                    lasersGame[i].transform.position = hit.point+new Vector3(0f,0.07f,0f);
                    lasersGame[i].transform.rotation = Quaternion.Euler(lasersGame[i].transform.parent.forward.x, 0f, lasersGame[i].transform.parent.forward.z); ;
                }
                else
                {
